...
const doc = ref<SupplierPayment | null>(null)
...
async function load() {
  loading.value = true
  try {
    const payment = await purchasingApi.getSupplierPayment(id.value)
    doc.value = payment
    const [accounts, openItems] = await Promise.all([
      accountingApi.accounts().catch(() => []),
      accountingApi.apOpenItems(payment.supplierId).catch(() => []),
    ])
      ...
      <div class="flex flex-wrap items-start justify-between gap-4">
        <div>
          <div class="flex items-center gap-3">
            ...
            <StatusBadge tone="info" :label="t('docDetail.paymentMadeTitle')" />
            <StatusBadge
        ...
        <button
          ...
          @click="router.push({ name: 'masterDataSupplierDetail', params: { id: doc.supplierId } })"
        >
          <ExternalLink :size="16" /> {{ t('docDetail.viewSupplier') }}
        </button>
      </div>

      <AppCard :title="t('docDetail.paymentMadeTitle')">
        ...
        <table v-if="doc.allocations.length" class="w-full text-sm">
          ...
          <tbody>
            <tr v-for="(a, i) in doc.allocations" :key="i" class="border-b border-border last:border-0">
              <td class="px-4 py-2.5 text-text">{{ openItemNames.get(a.apOpenItemId) ?? '—' }}</td>
