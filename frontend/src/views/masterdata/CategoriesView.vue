...
async function load() {
  loading.value = true
  try {
    rows.value = await masterData.productCategories()
  } finally {
    loading.value = false
  }
}
...
async function save() {
  ...
  saving.value = true
  try {
    if (editingId.value) await masterData.updateCategory(editingId.value, { name: form.name }, editRowVersion.value)
    else await masterData.createCategory({ code: form.code, name: form.name })
...
async function toggleActive(row: ProductCategory) {
  await masterData.setCategoryActive(row.id, !row.isActive)
  await load()
}
</script>
...
<template>
  <div class="space-y-4">
    <div class="flex justify-end">
      <AppButton v-if="auth.has('MasterData.Create')" @click="openNew"><Plus :size="16" /> {{ t('masterData.categories.new') }}</AppButton>
    </div>
    ...
    <DataTable searchable :columns="columns" :rows="rows" :loading="loading" :empty-text="t('masterData.empty')">
      ...
      <template #cell-actions="{ row }">
        <RowActions :row="(row as unknown as ProductCategory)" @edit="openEdit(row as unknown as ProductCategory)" @toggle="toggleActive(row as unknown as ProductCategory)" />
      </template>
    </DataTable>

    <AppModal v-model="modalOpen" :title="editingId ? t('masterData.categories.edit') : t('masterData.categories.new')">
