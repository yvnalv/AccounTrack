...
const form = reactive({ name: '', type: 'Sales' as PriceListType, isDefault: false, isActive: true })

function openNew() {
  editingId.value = null
  Object.assign(form, { name: '', type: 'Sales' as PriceListType, isDefault: false, isActive: true })
  error.value = ''
...
function openEdit(row: PriceList) {
  ...
  Object.assign(form, { name: row.name, type: row.type, isDefault: row.isDefault, isActive: row.isActive })
  error.value = ''
  modalOpen.value = true
}
  ...
  try {
    if (editingId.value) {
      await pricingApi.update(
        editingId.value,
        { name: form.name, isDefault: form.isDefault, isActive: form.isActive },
        editRowVersion.value,
      )
    } else {
      await pricingApi.create({ name: form.name, type: form.type, isDefault: form.isDefault })
    }
      ...
      <table class="w-full text-sm">
        <thead class="bg-surface-2 text-left text-xs text-text-muted">
          <tr>
            ...
            <th class="px-3 py-2 font-medium">{{ t('priceLists.itemsCount') }}</th>
        ...
        <tbody>
          <tr v-for="l in lists" :key="l.id" class="border-t border-border">
            <td class="px-3 py-2">
              ...
              <StatusBadge v-if="l.isDefault" class="ml-2" tone="info" :label="t('priceLists.default')" />
            </td>
            <td class="px-3 py-2 text-text-muted">
              {{ l.type === 'Sales' ? t('priceLists.type.sales') : t('priceLists.type.purchase') }}
            </td>
            <td class="px-3 py-2 text-text-muted tnum">{{ formatNumber(l.itemCount) }}</td>
            <td class="px-3 py-2">
    ...
    <AppModal v-model="modalOpen" :title="editingId ? t('priceLists.edit') : t('priceLists.new')">
      <div class="space-y-4">
        ...
        <label class="flex items-center gap-2 text-sm text-text">
          <input v-model="form.isDefault" type="checkbox" class="h-4 w-4 accent-accent" /> {{ t('priceLists.defaultHint') }}
        </label>
