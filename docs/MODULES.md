...
| Accounting | 🟡 Slice 1 + reports + posting engine | chart of accounts, fiscal periods (open/close/lock), double-entry journal posting + reversal, trial balance, **Profit & Loss + Balance Sheet** (derived from GL), **posting-rule / account-determination engine** (configurable, most-specific-wins, health check). Remaining slice 2: AR/AP subledgers, period-close snapshots, Cash Flow |
