...
| Inventory | 🟡 Slice 1 + 2 | transaction ledger (source of truth), moving-average buckets, receive/adjust/transfer, on-hand + stock-card; `IInventoryLedger`. **Slice 2 (CHG-0057):** stock adjustments + **opname** post Dr/Cr Inventory↔Variance to the GL atomically (transfers GL-neutral under one control account); **per-company negative-stock policy (CHG-0073)**. Remaining: back-dating recompute, FIFO option |
